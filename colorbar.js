export default function Colorbar(size, extent, w, { vertical = true,
    interpolater = d3.interpolateBlues,
    margin = { top: 10, right: 35, bottom: 30, left: 30 },
    update = function () { },
    pad = 8, rotate = -35, ticks = 4, titles = ["title"], titleSize = 16, fontSize = 11 }) {
    ...
    const colorbar_container = d3.create("div").classed("colorbar_container", true)

    // const select = colorbar_container
    //     .append("select")
    //     .style("position", "absolute")
    let [select, getSelect] = Dropdown(colorbar_container, titles, handleSelect)

    const svg = colorbar_container.append("svg")
    ...
    function handleSelect() {
        ...
        if (!vertical)
            axis.call(d3.axisBottom(y).tickValues(tickValues))
    }
    ...
    return colorbar_container.node()
...
function Dropdown(container, data, callback = function () { }) {
    ...
    let open = false, selected_option = data[0]
    ...
    const options_container = select.append("div")
